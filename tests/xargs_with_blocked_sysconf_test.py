...
def _create_import_test_script() -> str:
    """Create a Python script that tests importing pre_commit.xargs.

    Tests importing with blocked os.sysconf. This script must run in a
    subprocess (not in the main pytest process) because:
    ...
    return textwrap.dedent(f"""
        ...
        def main():
            ...
            # Block os.sysconf so that any SC_ARG_MAX lookup fails.
            with mock.patch.object(
                os, 'sysconf', side_effect=OSError('{BLOCKED_SYSCONF_ERROR}')
            ):
...
def test_import_module_without_throwing_exception_as_subprocess() -> None:
    ...
    - Phase 1 (this function): Sets up and runs a subprocess with blocked
      os.sysconf
    - Phase 2 (subprocess script): Attempts to import pre_commit.xargs in that
      environment

    With the current buggy implementation, the subprocess crashes because
    os.sysconf is called at import time. After the fix, the subprocess should
    succeed.
    """
    ...
    # Test-first: This test FAILS with current buggy behavior (subprocess
    # crashes). After fix, subprocess should succeed (returncode == 0).
    assert proc.returncode == 0, (
        f'Subprocess failed to import pre_commit.xargs when os.sysconf is '
        f'blocked. This exposes the bug: os.sysconf is called at import time. '
...
def _create_xargs_test_script(use_explicit_max_length: bool = False) -> str:
    ...
    This script must run in a subprocess (not in the main pytest process)
    because:
    1. We need to patch os.sysconf BEFORE any imports happen
    2. conftest.py already imports pre_commit.xargs in the main process
    3. A subprocess gives us a clean import state to test against
    ...
    return textwrap.dedent(f"""
        ...
        def main():
            ...
            with mock.patch.object(
                ...
                # Test that xargs actually works - call it with a simple cmd
...
def test_xargs_without_max_length_when_sysconf_blocked_as_subprocess() -> None:
    """Test xargs.xargs() works without _max_length when os.sysconf is blocked.

    This test verifies that lazy evaluation of _max_length handles blocked
    sysconf correctly. When _max_length is not provided, xargs should call
    _get_platform_max_length() at runtime, which should handle OSError
    gracefully.
...
def test_xargs_with_explicit_max_length_when_sysconf_blocked_as_subprocess(
) -> None:
    """Test xargs.xargs() works with explicit _max_length when sysconf blocked.

    This test verifies that when _max_length is provided explicitly, xargs
    works correctly even when os.sysconf is blocked. This tests the code path
    where _get_platform_max_length() is never called.
    ...
    assert proc.returncode == 0, (
        f'Subprocess failed to run xargs.xargs() with explicit _max_length '
        f'when os.sysconf is blocked. This should always work since '
        f'_get_platform_max_length() is never called. stderr: {proc.stderr!r}'
    )
